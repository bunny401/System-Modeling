...
import hu.bme.mit.inf.symod.IQULTE.homework.chessclockiqulte.ChessClockIQULTEStatemachine;
...
public class ReflectiveTimeInterfaceProvider {
	...
	public static Class<?> iTimer() {
		try {
			return Class.forName("hu.bme.mit.inf.symod.IQULTE.homework.ITimer");
	...
	public static Class<?> iTimerCallback() {
		if(isTimerAvailable()) {
			try {
				return Class.forName("hu.bme.mit.inf.symod.IQULTE.homework.ITimerCallback");
			} catch (ClassNotFoundException e) {
				//e.printStackTrace();
				return null;
			}
	...
	public static void setTimer(ChessClockIQULTEStatemachine machine, Object timer) {
	...
	public static Object providePhysicsTimer() {
		if(isTimerAvailable()) {
			try {
				return Class.forName("hu.bme.mit.inf.symod.IQULTE.homework.TimerService").newInstance();
